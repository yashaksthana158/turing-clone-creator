...
export default function Profile() {
  ...
  const [profile, setProfile] = useState<ProfileData>({
    full_name: '', phone: '', avatar_url: '', college: '', course: '', roll_no: '',
    admission_year: null, id_card_url: null, id_card_verified: false,
  });
  ...
  const fetchProfile = async () => {
    const { data, error } = await supabase
      .from('profiles')
      .select('full_name, phone, avatar_url, college, course, roll_no, admission_year, id_card_url, id_card_verified')
      .eq('id', user!.id)
      .single();
    ...
    if (!error && data) {
      setProfile({
        ...
        id_card_verified: data.id_card_verified ?? false,
      });
    }
    setLoading(false);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (trimmedPhone && !/^[+\d\s()-]{0,20}$/.test(trimmedPhone)) { toast.error('Invalid phone number format'); return; }

    setSaving(true);
    const { error } = await supabase.from('profiles').update({
      full_name: trimmedName, phone: trimmedPhone || null, updated_at: new Date().toISOString(),
    }).eq('id', user!.id);
  ...
  const renderIdBadge = () => {
    ...
    return (
      <button
        ...
        className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full bg-gray-500/20 text-gray-400 border border-gray-500/30 hover:bg-gray-500/30 transition-colors"
      >
        <AlertTriangle size={12} /> ID Not Uploaded
      </button>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-black py-12 px-4">
      <div className="max-w-2xl mx-auto space-y-6">
        <button onClick={() => navigate('/')} className="text-gray-400 hover:text-white transition-colors flex items-center gap-2 font-['Oxanium']">
        ...
        <div className="bg-[#1c1c1c] border border-[#d3d3d3] rounded-lg p-8">
          <div className="flex items-center justify-between mb-8">
            ...
            {!editing && (
              <button onClick={() => setEditing(true)} className="py-2 px-4 bg-[#9113ff] hover:bg-[#7c0fd9] text-white text-sm font-semibold rounded-lg transition-colors">
                Edit Profile
              </button>
            )}
          </div>
          ...
          <div className="flex items-center gap-6 mb-8">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {roles.map((role) => (
                  <span key={role} className={`text-xs px-2.5 py-1 rounded-full border ${roleBadgeColor[role] || 'bg-gray-500/20 text-gray-400 border-gray-500/30'}`}>
                    {role.replace('_', ' ')}
                  </span>
                ))}
              </div>
          ...
          {editing ? (
            <form onSubmit={handleSave} className="space-y-6">
              <div>
                ...
                <input type="text" value={profile.full_name} onChange={(e) => setProfile({ ...profile, full_name: e.target.value })} maxLength={100} className="w-full px-4 py-3 bg-black border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-[#9113ff] transition-colors" placeholder="Your full name" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Phone</label>
                <input type="tel" value={profile.phone} onChange={(e) => setProfile({ ...profile, phone: e.target.value })} maxLength={20} className="w-full px-4 py-3 bg-black border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-[#9113ff] transition-colors" placeholder="+91 1234567890" />
              </div>
              <div className="flex gap-3">
                <button type="submit" disabled={saving} className="py-3 px-6 bg-[#9113ff] hover:bg-[#7c0fd9] text-white font-semibold rounded-lg transition-colors disabled:opacity-50">
                  {saving ? 'Saving...' : 'Save Changes'}
                </button>
                <button type="button" onClick={() => { setEditing(false); fetchProfile(); }} className="py-3 px-6 bg-transparent border border-gray-600 text-gray-300 hover:bg-white/5 font-semibold rounded-lg transition-colors">
                  Cancel
                </button>
              </div>
            </form>
        ...
        <div className="bg-[#1c1c1c] border border-[#d3d3d3] rounded-lg p-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleSignOut} className="py-2 px-4 bg-red-500/20 text-red-400 hover:bg-red-500/30 border border-red-500/30 font-semibold rounded-lg transition-colors text-sm">
              Sign Out
            </button>
          </div>
        </div>
